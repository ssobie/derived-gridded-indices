...
run.metro.van.seasons <- function() {
  ds.type <- 'bccaq'
  var.list <- c('tasmax','tasmin') ##c('pr','tasmax','tasmin') ##'s30' ##
  past.int <- '1971-2000'
##  proj.int <- '2011-2040'
  scenario <- 'rcp85'
  
  proj.dir <-  '/storage/data/climate/downscale/BCCAQ2+PRISM/high_res_downscaling/bccaq_gcm_south_island_subset/'
